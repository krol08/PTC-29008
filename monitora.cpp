...
void Monitora::sentinela(){
	...
	if (mon_arq->get_TimerBackoff() == false){
		tv.tv_sec = 0;
		tv.tv_usec = 600000;
	}else{
		tv.tv_sec = 0;
		tv.tv_usec = mon_arq->get_backoff();
	}
	...
	if (ret_select == -1){
		...
		if (ret_select == 0){
			...
			if((this->mon_arq->get_flag_tun()) == false){
				if(this->mon_arq->get_TimerBackoff() == true){
					this->mon_arq->mef(0,0,Arq::BACKOFF);
					cout<< "Backoff esgotado! "<<endl;
				}else{
