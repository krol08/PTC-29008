...
void Monitora::sentinela(){
	...
	if (ret_select == -1){
		...
		if (ret_select == 0){
			...
			if((this->mon_arq->get_flag_tun()) == false){
				if(this->mon_arq->get_flag_controle() == true){
					this->mon_arq->mef_mestre(0, 0, Arq::TIMEOUT);
				}else{
					this->mon_arq->mef_escravo(0, 0, Arq::TIMEOUT);
				}
			...
			if(FD_ISSET(_fd_serial,&_readset)){
			    ...
			    if (num_bytes > 0){
			    	...
			    	switch(buffer[0]){
			    		case Arq::ACK:
			    			if(this->mon_arq->get_flag_controle() == true){
			    				this->mon_arq->mef_mestre(buffer, num_bytes, Arq::ACK);
				    		}else{
				    			this->mon_arq->mef_escravo(buffer, num_bytes, Arq::ACK);
				    		}
			    		break;
			    		...
			    		case Arq::FRAME:
			    			...
			    			if(this->mon_arq->get_flag_controle() == true){
			    				this->mon_arq->mef_mestre(buffer, num_bytes, Arq::FRAME);
			    			}else{
			    				this->mon_arq->mef_escravo(buffer, num_bytes, Arq::FRAME);
			    			}
			    		break;

			    		case Arq::POLL:
			    			this->mon_arq->mef_escravo(0, 0, Arq::POLL);
			    		break;

			    		case Arq::POLL_END:
		    				this->mon_arq->mef_mestre(0, 0, Arq::POLL_END);
			    		break;
			    	}
			...
			if(FD_ISSET(_fd_tun,&_readset)){
    			...
    			if(this->mon_arq->get_flag_controle() == true){
    				this->mon_arq->mef_mestre(buffer,num,Arq::PAYLOAD);
    			}else{
    				this->mon_arq->mef_escravo(buffer,num,Arq::PAYLOAD);
    			}
			}
		}			
	}
